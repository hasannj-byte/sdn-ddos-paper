...
\begin{table*}[hbt!]
\centering
\caption{Closed-loop mitigation on the testbed: control-plane behavior with the defense disabled versus enabled. \textit{Pending the Mininet/POX/Open vSwitch testbed run.}}
...
\hline
Attack, defense OFF & N/A & N/A & \textit{pending} & \textit{pending} & \textit{pending} \\ \hline
\rowcolor{gray!20}
\textbf{Attack, defense ON} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} \\ \hline
\end{tabular}
\end{table*}
...
\begin{table*}[hbt!]
\centering
\caption{Cross-dataset evaluation: zero-shot transfer and after a brief adaptation step (5\% of the target traffic). Accuracy and F1 are in $[0,1]$. The live-testbed row awaits the closed-loop testbed run.}
...
\rowcolor{gray!20}
CICDDoS2019 $\rightarrow$ Testbed (live) & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} \\ \hline
\end{tabular}
\end{table*}

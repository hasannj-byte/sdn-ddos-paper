...
%TODO: numeric entries marked [XX] and the regenerated figures below are filled from the re-run experiments (leakage-free protocol). They replace the earlier same-dataset numbers, which were produced with SMOTE applied before the split and on a rebalanced test set.
...
\begin{table*}[hbt!]
\centering
\caption{Detection performance on the CICDDoS2019 test set (natural class distribution). Baselines re-implemented on the identical split. Bracketed values [XX] are filled from the re-run experiments.}
...
The confusion matrix in Figure~\ref{fig:confusion_matrix} breaks the predictions down into the four outcomes on the held-out test set, which keeps its natural class ratio rather than the balanced one used previously. The model labels [XX] benign flows correctly (true negatives) and misclassifies [XX] benign flows as attacks (false positives), while it detects [XX] attack flows (true positives) and misses [XX] (false negatives). Because the test set is imbalanced, the small false-positive count matters more than a raw accuracy figure would suggest: it is what keeps the false positive rate at [XX]\% and avoids flooding the operator with spurious alerts on the much larger benign class.
...
The area under the ROC curve is [XX]. Since ROC-AUC can look optimistic on imbalanced data, we read it together with the PR-AUC of [XX] from Table~\ref{tab:detection_results}; the two agree that the model separates the classes well across thresholds while holding the false positive rate low. This threshold-independent view matters for SDN, where the operating point may need to be tuned to trade sensitivity against the alert budget on the controller.
...
\begin{table*}[hbt!]
\centering
\caption{Behavior under concept drift: static model versus the proposed adaptive model on the streamed evaluation. Bracketed values [XX] are filled from the re-run experiments.}
...
\hline
Static model (no update) & [XX] & [XX] & [XX] & N/A & N/A \\ \hline
Adaptive, no replay buffer & [XX] & [XX] & [XX] & [XX] & [XX] \\ \hline
\rowcolor{gray!20}
\textbf{Adaptive + replay (proposed)} & [XX] & [XX] & [XX] & [XX] & [XX] \\ \hline
\end{tabular}
\end{table*}

The expected pattern, to be confirmed by the filled values, is that the static model loses accuracy at each drift point and does not recover, while the adaptive model dips briefly and then returns to its earlier level within [XX] updates. The replay buffer is what keeps the forgetting measure low: without it, the model recovers on the newest family but loses ground on the ones seen before, which is visible as the gap between the second and third rows of Table~\ref{tab:drift_results}.

\subsection{Closed-Loop Mitigation and Controller Overhead}
\label{sec:mitigation_results}

The second capability is acting on the network. Using the mitigation policy of Section~\ref{sec:mitigation}, we run the testbed under attack with the defense disabled and then enabled, and measure the effect on the control plane. Table~\ref{tab:mitigation_results} reports the time from attack onset to detection and to rule installation, the rate of \texttt{Packet-In} messages reaching the controller, controller CPU and memory, and the goodput and round-trip time seen by legitimate hosts.

\begin{table*}[hbt!]
\centering
\caption{Closed-loop mitigation on the testbed: control-plane behavior with the defense disabled versus enabled. Bracketed values [XX] are filled from the re-run experiments.}
\label{tab:mitigation_results}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|p{4.0cm}|c|c|c|c|c|}
\hline
\textbf{Condition} & \textbf{Time-to-detect (ms)} & \textbf{Time-to-mitigate (ms)} & \textbf{Peak \texttt{Packet-In}/s} & \textbf{Controller CPU (\%)} & \textbf{Legit. goodput} \\
\hline
Attack, defense OFF & N/A & N/A & [XX] & [XX] & [XX] \\ \hline
\rowcolor{gray!20}
\textbf{Attack, defense ON} & [XX] & [XX] & [XX] & [XX] & [XX] \\ \hline
\end{tabular}
\end{table*}

The point of these numbers is to make the ``real-time'' claim measurable. With the defense enabled, the surge of \texttt{Packet-In} messages should collapse shortly after the rule is installed, the controller load should fall back toward its idle level, and legitimate hosts should keep most of their goodput while the attack is in progress. The per-flow inference latency from Table~\ref{tab:detection_results} and the time-to-mitigate here together bound how quickly the system reacts, and the gap between the two rows quantifies the overhead the detector adds when it is doing its job.
...
\begin{table*}[hbt!]
\centering
\caption{Cross-dataset evaluation: zero-shot transfer and after brief adaptation. Bracketed values [XX] are filled from the re-run experiments.}
...
\hline
CICDDoS2019 $\rightarrow$ CICDDoS2019 (ref.) & [XX] & [XX] & --- & --- \\ \hline
CICDDoS2019 $\rightarrow$ InSDN & [XX] & [XX] & [XX] & [XX] \\ \hline
\rowcolor{gray!20}
CICDDoS2019 $\rightarrow$ Testbed (live) & [XX] & [XX] & [XX] & [XX] \\ \hline
\end{tabular}
\end{table*}
...
Several limitations qualify these results and shape the conclusions we draw. The online adaptation assumes that ground-truth labels are available for a fraction of recent traffic, which is realistic only where detections are confirmed by an operator or a slower offline verifier; under a tight label budget the update cadence, and therefore the recovery time in Table~\ref{tab:drift_results}, will be longer than reported, and we treat the label budget as an explicit parameter rather than a solved problem. The testbed is an emulation on a single host, so the absolute latency and throughput figures reflect that scale and would change on production hardware and topologies, even if the relative effect of mitigation should carry over. The cross-dataset study depends on mapping CICDDoS2019 and InSDN onto a shared feature subset; differences in how each dataset is captured and labeled place a ceiling on transfer that no model can fully overcome. Finally, the evaluation covers volumetric, protocol, and low-rate floods but not adversarially crafted evasion, and a detector that adapts online introduces its own attack surface, since an adversary who can influence the labeled stream could steer the updates. We regard adversarial robustness of the adaptive loop as the most important item for future work, alongside scaling the testbed and reducing the label requirement of the update step.



\section{Conclusion}
\label{sec:conclusion}
SDN improves network adaptability through centralized control, but the same design makes the controller a high-value target for DoS and DDoS attacks. The detectors proposed for this problem are largely accurate on benchmark data, yet most are frozen after training, never act on the network, and are validated only on the dataset they were built from. This work keeps a compact CNN-LSTM model, roughly 80,000 parameters over eight flow-level features, and reorganizes the rest of the system around those three gaps.

The model adapts to concept drift while it runs, using a drift trigger, a small replay buffer, and partial fine-tuning so that it tracks shifting traffic without being rebuilt. It operates in closed loop with a POX controller, installing OpenFlow rules to drop or rate-limit malicious sources, and we report the operational cost of doing so rather than assuming it. It is evaluated across datasets, trained on CICDDoS2019 and tested on InSDN and on live captures from a Mininet, POX, and Open vSwitch testbed, with the transfer gap and its recovery after a short adaptation step both reported. Throughout, balancing is confined to the training folds and results are reported on the natural class distribution with PR-AUC and MCC alongside accuracy, which removes the inflation that a leak-before-split protocol introduces.

The detection accuracy on CICDDoS2019 is competitive with the strongest hybrids rather than a new record, and we state it as such. The contribution is that comparable detection quality is delivered together with adaptivity, an instrumented mitigation loop, and honest cross-dataset evaluation, each at a measured cost. The most pressing direction for future work is the adversarial robustness of the adaptive loop, since a model that learns online can also be misled online; scaling the testbed beyond single-host emulation and lowering the label requirement of the update step follow close behind. The implementation will be released to support replication, and the data availability statement below describes how to obtain the data used here.

...
\begin{document}
\title{A Lightweight Hybrid CNN-LSTM Approach for Real-Time DoS/DDoS Detection in Software-Defined Networks}
%\subtitle{Subtitle}
\transtitle{A Lightweight Hybrid CNN-LSTM Approach for Real-Time DoS/DDoS Detection in Software-Defined Networks}
...
\keywords{Software-defined networking | DDoS Detection | Network Security | Deep Learning | Hybrid CNN-LSTM model}

\transkeywords{DEM | flexible cylindrical particle | internal friction angle | particle deformation | shear stress}

\abstract[ABSTRACT]{Software-Defined Networking (SDN) optimizes network flexibility through centralized control. Nevertheless, its centralized structure makes it susceptible to Distributed Denial-of-Service (DDoS) attacks. In particular, the SDN controller becomes a key target of DDoS attack. To address this challenge, a lightweight hybrid deep learning (DL) model is proposed that combines Convolutional Neural Networks (CNN) and Long Short-Term Memory (LSTM). CNN extracts the spatial traffic features while the LSTM component captures temporal patterns to improve attack detection rate. The proposed model contains only 80,000 trainable parameters and employs eight key traffic features, achieving better detection performance with limited computational cost. The model is trained using the CICDDoS2019 dataset and is evaluated in a Mininet-based SDN environment using POX, Open vSwitch, and sFlow for real-time traffic export. A balanced data sampling method (SMOTE, undersampling), early stopping, and adaptive learning rate strategies are used to minimize the training time and computational cost.The proposed model achieves accuracy of 99.5\% and F-score of 99.5\% which outperforms the existing machine learning and deep learning baselines including  including RF (91.1\%), CNN-AD (97.4\%), DT (94.9\%), SVM (92.7\%), KNN (96.6\%), and NB (71.1\%). In addition, the proposed model has shown better behavior in various optimization conditions, with Adam showing better results than SGD and a batch size of 64 showing better results compared to 128. Existing methods often focus on offline testing or require high computational cost. The proposed algorithm establishes efficient processing, making it suitable for real-time deployment on SDN.}
...
In order to address the growing challenge of DDoS attacks in SDN environments, this research proposes a lightweight hybrid deep learning (DL) model that combines efficiency with effectiveness. The model is designed for real-time detection and minimal resource consumption, making it suitable for practical deployment in programmable networks. By utilizing a selected subset of eight flow-based features, The model minimizes complexity while keeping scalability and transparency without compromising performance. The proposed model is optimized to 80,000 parameters that shows a balance among accuracy and competency for resource-constrained domains. 
...
The proposed model is validated by using the benchmark dataset named CICDDoS2019 \cite{cicddos2019dataset}. This dataset covers a wide range of modern DDoS attack types across multiple protocols, including HTTP flood, UDP flood, TCP SYN flood, DNS amplification, and LDAP reflection attacks. The model is also tested on a real-time emulated SDN set-up using Mininet. POX acts as the SDN controller, and Open vSwitch handles packet forwarding. The sFlow captures and monitors traffic from the SDN network. This setup ensures the evaluation reflects real deployment conditions and not just theoretical experiments. The data preprocessing stage uses SMOTE and random undersampling to balance class distribution \cite{ratnasari2024performance}. The training process applies adaptive learning rate scheduling and early stopping to prevent overfitting. Moreover, proposed model is validated by using various training configurations that consists of different optimizers such as Adam and SGD and varying batch sizes (e.g., 64 and 128), to analyze performance robustness and generalization capability.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%\subsection{Performance Evaluation and Results}
The proposed approach is proven to be effective as shown by experimental results with an accuracy of 99.5\%, a F-score of 99.4\%, and a false positive rate of 0.076. Furthermore, we validated proposed model with different optimizers and batch sizes where Adm and batch size 64 outperforms SGD and batch size 128 respectively. These findings show that the model can effectively discriminate normal and malicious traffic, and that it is highly generalized in various attack scenarios. Lightweight design also makes it applicable when deploying SDN in real time.


The key contributions of the work include:
\begin{itemize}
    \item Lightweight hybrid CNNLSTM model is proposed to detect DoS/DDoS attacks in SDN with high detection accuracy and less computational load.
    \item A more efficient feature selection mechanism is incorporated in order to reduce the complexity of calculations and improve the efficiency of the model.
    \item Experimental results demonstrates that the presented model is better than baseline machine learning and deep learning models, including SVM, RF, NB, and KNN, as it has a higher accuracy, F1-score, and a lower false positive rate.
    \item  To ensure model stability and training efficiency, hyperparameter analysis with various optimizers (Adam and SGD) and batch sizes (64 and 128) is performed. \end{itemize}

...
The entire testbed runs on a dedicated Ubuntu 24.04 LTS virtual machine (KVM/QEMU via Proxmox VE) rather than a nested Windows/VMware guest, so Mininet's process-based emulation runs natively on the host kernel. The controller logic and the CNN-LSTM model operate within this same environment for consistency. Python 3.12 is used for development. The key libraries include TensorFlow, Scikit-learn, NumPy, Pandas, XGBoost, and imbalanced-learn. Table \ref{table:sdn_setup} provides a summary of the hardware and software specifications used in the testbed.
...
1 & Processor & Intel(R) Core(TM) Ultra 9 285, 16 vCPU \\ \hline
2 & Installed RAM & 48 GB \\ \hline
3 & System Type & 64-bit OS, x86\_64 \\ \hline
4 & Storage & 300 GB SSD (virtual disk) \\ \hline
5 & GPU & NVIDIA RTX 5080, 16 GB VRAM (visible to the OS; not used for training -- see Section~\ref{sec:threats}) \\ \hline
\multicolumn{3}{|c|}{\textbf{Operating System / Virtualization Setup}} \\ \hline
6 & Host Hypervisor & Proxmox VE (KVM/QEMU) \\ \hline
7 & Guest OS & Ubuntu 24.04.4 LTS \\ \hline
\multicolumn{3}{|c|}{\textbf{SDN Tools and Controllers}} \\ \hline
8 & Mininet Version & 2.3.0-1.1 \\ \hline
9 & Open vSwitch Version & 3.3.4 (kernel datapath) \\ \hline
10 & OpenFlow Version & 1.0 (\texttt{openflow.of\_01}; POX's default module) \\ \hline
11 & POX Controller Version & POX 0.7.0 (gar) / Python 3.12.3 \\ \hline
12 & Monitoring Integration & sFlow (sflowtool, built from source) for flow export \\ \hline
\multicolumn{3}{|c|}{\textbf{Traffic Simulation}} \\ \hline
13 & Normal Traffic & ICMP and periodic short TCP bursts between Mininet hosts \\ \hline
14 & Attack Traffic & SYN flood, UDP flood, and low-rate (Shrew-style) pulses via hping3 \\ \hline
\multicolumn{3}{|c|}{\textbf{ML and DL Frameworks}} \\ \hline
15 & Python Version & Python 3.12.3 \\ \hline
16 & Libraries & TensorFlow 2.17, Scikit-learn, Pandas, NumPy, XGBoost, Imbalanced-learn, SciPy \\ \hline
\multicolumn{3}{|c|}{\textbf{Dataset Used}} \\ \hline
17 & Dataset & CICDDoS2019 (detection), InSDN and a live sFlow capture (cross-dataset) \\ \hline
\end{tabular}
}
...
Table~\ref{tab:detection_results} reports a single run (seed 42) for comparability with the rest of the paper's tables. To confirm the ranking is not an artifact of that one run, every model was retrained across 5 seeds (42--46) on the identical split, varying only the training-time randomness (weight initialization, batch order, and tree random state); Table~\ref{tab:detection_stats} reports the resulting mean $\pm$ standard deviation for accuracy and MCC.

\begin{table}[hbt!]
\centering
\caption{Accuracy and MCC, mean $\pm$ std over 5 training seeds (42--46), identical split. Accuracy in percent, MCC in $[0,1]$.}
...
\hline
Random Forest & 99.981 $\pm$ 0.000 & 0.9834 $\pm$ 0.0002 \\ \hline
XGBoost & 99.995 $\pm$ 0.000 & 0.9953 $\pm$ 0.0003 \\ \hline
CNN (1D) & 99.926 $\pm$ 0.009 & 0.9408 $\pm$ 0.0064 \\ \hline
LSTM & 99.961 $\pm$ 0.006 & 0.9670 $\pm$ 0.0045 \\ \hline
CNN-LSTM (full) & 99.962 $\pm$ 0.007 & 0.9678 $\pm$ 0.0053 \\ \hline
Transformer (small) & 99.931 $\pm$ 0.023 & 0.9445 $\pm$ 0.0171 \\ \hline
\rowcolor{gray!20}
\textbf{Proposed (8 feat.)} & \textbf{99.962 $\pm$ 0.007} & \textbf{0.9678 $\pm$ 0.0053} \\ \hline
\end{tabular}
\end{table}

The ranking is stable across seeds: XGBoost remains the strongest model on MCC and the proposed model remains mid-pack, matching Table~\ref{tab:detection_results}'s single-run reading. A paired comparison against XGBoost -- the strongest baseline -- on MCC across the same 5 seeds gives a paired $t$-test of $p = 0.0003$, rejecting the null that the two models perform equally; a Wilcoxon signed-rank test on the same pairs gives $p = 0.0625$, which does not reach significance at $n=5$ (the maximum-power configuration of that test, all five differences in the same direction, is itself only borderline significant at this sample size). We read this as convergent evidence with a genuine caveat: the parametric test says the gap is real, the non-parametric one is underpowered rather than contradicting it, and five seeds is the practical minimum for either test to say anything at all. Both tests agree in direction and neither supports the proposed model being the more accurate detector, which is consistent with the paper's framing throughout -- the contribution is adaptivity, closed-loop mitigation, and cross-dataset evaluation at a measured cost, not a new accuracy record.

\subsection{Optimization Analysis of the Proposed Model}
Figure~\ref{fig:p4} reports the effect of the optimizer and batch size on the CNN-LSTM model on CICDDoS2019 (Adam/batch-64 is the 5-seed mean from Table~\ref{tab:detection_stats}; the other three configurations are single runs at seed 42 on the same split). Across the configurations tested, Adam gives higher accuracy and F1-score than SGD at both batch sizes (99.96\% vs.\ 99.92--99.93\% accuracy), and batch size 64 edges out 128 for a given optimizer. Adam with batch size~64 reaches 99.96\% accuracy and a 99.98\% F1-score. The gap between the best and worst configuration is under 0.05 percentage points, so the choice affects the result at the margin rather than qualitatively, but Adam is the consistently better optimizer here. We therefore use Adam with a batch size of 64 in all other experiments.
\begin{figure*}[hbt!]
\centering
...
The second capability is acting on the network. Using the mitigation policy of Section~\ref{sec:mitigation}, we run the testbed (Mininet, POX, Open vSwitch, single host emulation) under three attack types -- SYN flood, UDP flood, and a low-rate Shrew-style pulse -- with the defense disabled and then enabled, and measure the effect on the control plane. This is the second of two testbed runs: the first (an earlier revision of this section) used a fixed 50-packet detection window and UDP benign traffic, and its results are superseded here after two fixes described in Section~\ref{sec:threats} -- a wall-clock detection window and genuine TCP benign traffic -- changed the outcome. Table~\ref{tab:mitigation_results} reports the peak rate of \texttt{Packet-In} messages reaching the controller, controller CPU and memory, the classification-to-rule-install latency, and the legitimate client's completed TCP bursts and goodput.

\begin{table*}[hbt!]
\centering
\caption{Closed-loop mitigation on the testbed (post-fix run): control-plane behavior with the defense disabled versus enabled, across three attack types. \textbf{Classify$\to$rule (ms)} is N/A throughout because no source crossed the mitigation threshold in this run (see discussion below) -- it is not a verified attack-onset-to-response time even where it fires, since no independent attack-start marker is recorded. \textbf{TCP bursts} is completed$/$expected short TCP transfers from the benign client, a genuine delivery signal (a blocked client's bursts simply never complete). \textbf{Goodput} averages only the completed bursts.}
\label{tab:mitigation_results}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|p{2.3cm}|c|c|c|c|c|c|}
\hline
\textbf{Condition} & \textbf{Classify$\to$rule (ms)} & \textbf{Peak \texttt{Packet-In}/s} & \textbf{Ctrl. CPU (\%)} & \textbf{Ctrl. RSS (MB)} & \textbf{TCP bursts} & \textbf{Goodput (Mbps)} \\
\hline
SYN, OFF & N/A & 635{,}076 & 79.4 & 47 & 4/15 & 10.4 \\ \hline
SYN, ON & N/A & 578{,}535 & 79.6 & 561 & 4/15 & 17.5 \\ \hline
UDP, OFF & N/A & 607{,}699 & 79.3 & 48 & 3/15 & 5.0 \\ \hline
UDP, ON & N/A & 564{,}732 & 79.4 & 561 & 4/15 & 9.8 \\ \hline
Low-rate, OFF & N/A & 2{,}976 & 0.4 & 42 & 16/15 & 20.5 \\ \hline
Low-rate, ON & N/A & 2{,}251 & 2.9 & 560 & 15/15 & 18.6 \\ \hline
\end{tabular}
\end{table*}

With both fixes applied, the false positive that the earlier run reported -- mitigating the benign host instead of the attacker, across all three attack types -- is gone: zero mitigation events occur across all six conditions in this run. That is real progress and we report it as such, but it comes with an unavoidable consequence for this table: with no rule ever installed, ON and OFF differ only in whether the controller also runs classification, not in whether anything gets blocked, so the Packet-In-rate and goodput columns are run-to-run variance under contention rather than a measured effect of mitigation, and \textbf{Classify$\to$rule is N/A throughout because the event never occurs in this run}. The rule-installation mechanism itself is not hypothetical -- it fired correctly and within a fraction of a millisecond in the earlier run once a classification crossed the threshold -- but this specific run does not exercise it, because the underlying detection gap traced in Section~\ref{sec:threats} means the actual attacker in all three attack types is never classified with enough confidence to trigger it. What this table does show cleanly is the controller-side cost of running the classifier continuously: resident memory rises by roughly 510--520\,MB (loading the model and scaler) regardless of attack type, while CPU overhead from classification itself is small enough to be within run-to-run noise for the two high-volume floods and only becomes visible, proportionally, under the otherwise-idle low-rate condition (0.4\%\ vs.\ 2.9\%). The completed-burst counts are themselves informative independent of mitigation: legitimate TCP traffic is substantially disrupted by the SYN and UDP floods regardless of whether the defense is on (3--4 of 15 bursts complete, against 15--16 of 15 under the low-rate attack), which is a real, if unintended, illustration of why an attack that isn't correctly blocked still costs the network something.
...
Several limitations qualify these results and shape the conclusions we draw. The most consequential is specific to the closed-loop testbed and took two iterations to characterize properly. The controller classifies each source from a live per-source packet window (Section~\ref{sec:mitigation}), and an initial version of this window -- closing after a fixed count of 50 packets -- did not match the offline CICFlowMeter features the model was trained on: because Mininet's virtual switching delivers 50 packets to the controller in one to two milliseconds regardless of the underlying application-level send rate, \emph{Flow Duration} collapsed to values with no relationship to real flow durations, and the controller ended up mitigating the benign traffic generator rather than the attacking hosts across all three attack types. Replacing the fixed packet count with a wall-clock window (Section~\ref{sec:mitigation}, \texttt{detect\_window\_seconds}) fixed this specific failure: live \emph{Flow Duration} now reads approximately one second as intended, and combined with switching the benign traffic generator from UDP to genuine TCP -- UDP benign traffic turned out to share CICDDoS2019's zeroed-out ACK-flag and initial-window fields with its own UDP-based attack families, leaving the model no real signal to separate them -- the false positive is gone entirely: zero mitigation events occurred across all six conditions behind the current Table~\ref{tab:mitigation_results}. A second, independent problem remains and is not fixed by either change: the actual attacking hosts are still never classified with enough confidence to trigger mitigation, in any of the three attack types. We traced this to CICDDoS2019's own \emph{Flow Packets/s} column being dominated by a small number of extreme outlier rows, which makes the training-fit \texttt{StandardScaler}'s mean (on the order of $10^6$ packets/s) far higher than any traffic our testbed actually generates, so a genuine flood's packet rate still scales to a value that reads as below-average rather than anomalous. Correcting this would require retraining with different feature preprocessing -- robust scaling or explicit outlier handling on that column -- which would change the model behind the already-verified Table~\ref{tab:detection_results} and Table~\ref{tab:detection_stats} and is out of scope for this revision; we report it as the top remaining item rather than attempt a fix that would put those tables' validity at risk. The mitigation \emph{mechanism} itself -- classification, OpenFlow rule installation on a positive decision, and the resulting drop in controller load -- is not hypothetical: it fired correctly and within a fraction of a millisecond of a classification in the earlier (pre-fix) run, before the wall-clock window and TCP traffic changes were made. What we can no longer claim, with the improved and more defensible pipeline, is that the testbed demonstrates the mechanism being triggered by a correct decision, because in this run it is never triggered at all. The online adaptation assumes that ground-truth labels are available for a fraction of recent traffic, which is realistic only where detections are confirmed by an operator or a slower offline verifier; under a tight label budget the update cadence, and therefore the recovery time in Table~\ref{tab:drift_results}, will be longer than reported, and we treat the label budget as an explicit parameter rather than a solved problem. The testbed is an emulation on a single host, so the absolute latency and throughput figures reflect that scale and would change on production hardware and topologies. The live-testbed row of Table~\ref{tab:cross_dataset_results} is built from a 146-flow capture, small enough that it should be read as indicative rather than precise, and its poor zero-shot performance is plausibly a symptom of the same outlier-driven scaling issue described above rather than an independent generalization finding. The cross-dataset study more broadly depends on mapping CICDDoS2019, InSDN, and the testbed capture onto a shared feature subset; differences in how each is captured and labeled place a ceiling on transfer that no model can fully overcome. The sFlow-based offline feature extractor used for the testbed capture also carries disclosed approximations -- direction is assumed forward-only and TCP window/header length default to fixed values where sFlow's line output does not expose them, verified only in that it does not silently mislabel fields (see \texttt{feature\_extractor.py}). Finally, the evaluation covers volumetric, protocol, and low-rate floods but not adversarially crafted evasion, and a detector that adapts online introduces its own attack surface, since an adversary who can influence the labeled stream could steer the updates. We regard retraining with outlier-robust feature scaling -- so that live traffic can actually be classified as anomalous on the packet-rate axis, closing the remaining mitigation-targeting gap -- as the most pressing next step, ahead of adversarial robustness of the adaptive loop, scaling the testbed, and reducing the label requirement of the update step.
